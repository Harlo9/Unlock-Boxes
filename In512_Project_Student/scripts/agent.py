...
class Agent:
    ...
    def __init__(self, server_ip):
        ...
        #self.direction = 2  # Default movement direction (e.g., 2 = right)
    ...
    #on veut juste que les robot bouge 
    def test_move(self):
        """ Send a move command to move the robot one step """
        print("Staring a test to move")
        cmds = {
            "header": MOVE,
            "direction": 3  # déplace vers la droit
        }
        self.network.send(cmds)
        print(f"Agent {self.agent_id} moving in direction {cmds['direction']}")
